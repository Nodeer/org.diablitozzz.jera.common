...
public class JsonDateISO8601 {
    ...
    public static String format(final Calendar cal) throws IllegalArgumentException {
        ...
        // year ([-]YYYY)
        JsonDateISO8601.appendZeroPaddedInt(buf, JsonDateISO8601.getYear(cal), 4);
        buf.append('-');
        // month (MM)
        JsonDateISO8601.appendZeroPaddedInt(buf, cal.get(Calendar.MONTH) + 1, 2);
        buf.append('-');
        // day (DD)
        JsonDateISO8601.appendZeroPaddedInt(buf, cal.get(Calendar.DAY_OF_MONTH), 2);
        buf.append('T');
        // hour (hh)
        JsonDateISO8601.appendZeroPaddedInt(buf, cal.get(Calendar.HOUR_OF_DAY), 2);
        buf.append(':');
        // minute (mm)
        JsonDateISO8601.appendZeroPaddedInt(buf, cal.get(Calendar.MINUTE), 2);
        buf.append(':');
        // second (ss)
        JsonDateISO8601.appendZeroPaddedInt(buf, cal.get(Calendar.SECOND), 2);
        buf.append('.');
        // millisecond (SSS)
        JsonDateISO8601.appendZeroPaddedInt(buf, cal.get(Calendar.MILLISECOND), 3);
        ...
        if (offset != 0) {
            ...
            buf.append(offset < 0 ? '-' : '+');
            JsonDateISO8601.appendZeroPaddedInt(buf, hours, 2);
            buf.append(':');
            JsonDateISO8601.appendZeroPaddedInt(buf, minutes, 2);
        } else {
            buf.append('Z');
        }
        return buf.toString();
